...
from lightrft.models import ScalarRewardModelVL, concatenated_forward
from lightrft.datasets import RankDatasetVL


T2I_TASK_INSTRUCTION="""Your will act as an expert image evaluator for text-to-image generation.
...
class HPDv3Evaluator(BaseScalarEvaluator):
    """
    ...
    def evaluate_batch(self, scores0_batch: Dict[str, torch.Tensor], scores1_batch: Dict[str, torch.Tensor], extras: Dict[str, Any]) -> None:
        ...
        for i in range(batch_size):
            ...
            print(f"Sample {i} | S0: {addup_scores0:.4f} vs S1: {addup_scores1:.4f} => {gt_choice} | Correct: {is_correct}")
            ...
            self.results.append({
                ...
                "prompt": extras[i]['prompt'],
                "model_chosen": extras[i]['model_chosen'],
                "model_rejected": extras[i]['model_rejected'],
                "preferred_path": extras[i]["preferred_path"],
                "rejected_path": extras[i]["rejected_path"],
            })
...
class GenAIBenchEvaluator(BaseScalarEvaluator):
    """
    Evaluator for GenAI-Bench (Image or Video) dataset.
    """

    def evaluate_batch(self, scores0_batch: Dict[str, torch.Tensor], scores1_batch: Dict[str, torch.Tensor],
        ...
        for i in range(batch_size):
            ...
            for head in head_names:
                ...
                # Collect data for global histogram
                self.per_head_scores[head].append(val0)
                self.per_head_scores[head].append(val1)
            ...
            res = {
                ...
                "is_correct": is_correct,
                "prompt": extras[i]['prompt'],
                "model1": extras[i]['model1'],
                "model2": extras[i]['model2'],
            }
            if 'index' in extras[i]:
                res['index'] = extras[i]['index']
            if 'id' in extras[i]:
                res['id'] = extras[i]['id']
            self.results.append(res)


class ImageRewardDBEvaluator(BaseScalarEvaluator):
    """
    Evaluator for ImageRewardDB Test set.
    """
    ...
    def evaluate_batch(self, scores0_batch: Dict[str, torch.Tensor], scores1_batch: Dict[str, torch.Tensor], extras: Dict[str, Any]) -> None:
        ...
        for i in range(batch_size):
            ...
            self.results.append({
                ...
                "is_correct": is_correct,
                "prompt": extras[i]['prompt'],
                "chosen_img": extras[i]["chosen_img"],
                "rejected_img": extras[i]["rejected_img"],
                "rank_chosen": extras[i]["rank_chosen"],
                "rank_rejected": extras[i]["rank_rejected"],
                "overall_rating_chosen": extras[i]["overall_rating_chosen"],
                "overall_rating_rejected": extras[i]["overall_rating_rejected"],
                "classification": extras[i]["classification"],
            })

@torch.no_grad()
def test_scalar_rm(
    model: ScalarRewardModelVL,
    ...
    for idx, batch in enumerate(tqdm(data_loader)):
        (
            ...
            input0_img_pixels, input0_img_grid_thws,
            input1_img_pixels, input1_img_grid_thws,
            input0_video_pixels, input0_video_grid_thws,
            input1_video_pixels, input1_video_grid_thws,
            extras
        ) = batch
        ...
        scores0, scores1 = concatenated_forward(
            model,
            input0_ids, input0_mask,
            input1_ids, input1_mask,
            input0_img_pixels, input0_img_grid_thws,
...
if __name__ == "__main__":
    model_path = "paths/to/your/saved/model"
    
    # Common model configuration (assumed same for all benchmarks in this run)
    model_config = {
        "pooling_method": "attn",
    ...
    model = ScalarRewardModelVL(
        ...
        scale_for_train=model_config["scale_for_train"],
        probing_layer=model_config["probing_layer"],
        head_types=model_config["head_types"],
    )
    ...
    max_pixels = 768 * 480  # Example max pixels for image inputs
    benchmark_configs = [
        {
            "name": "HPDv3-Test",
