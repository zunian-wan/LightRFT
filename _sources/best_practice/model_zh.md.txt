...
**核心特性**：
- **多模态能力**：模态声明为 `ActorModality.AUDIO_LANGUAGE`，处理音频和文本的组合输入。
- **架构适配**：支持如 Qwen2-Audio 等多种音频语言模型架构。
- **优化与 MoE**：支持混合专家 (MoE) 模型，通过梯度检查点进行显存优化，以及通过 LoRA 和 Flash Attention 提升效率。
...
print(grm_output)
# 输出: "<reasoning>The response accurately describes the content of the image.</reasoning><score>5</score>" -> 输出文本推理过程与评分
```
