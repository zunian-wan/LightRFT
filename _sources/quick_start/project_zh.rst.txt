...
Models 模块定义了语言模型和视觉语言模型进行强化学习所需的神经架构和适配。它专注于通过 monkey patch 方法对现有模型架构进行最小限度的修改。
...
5. **经验生成**：系统利用 Strategy 模块集成的推理引擎（如 vLLM/SGLang）进行高效的大规模并行采样，生成用于训练的经验数据 (Rollout)。

6. **模型优化**：Trainer 模块基于生成的经验计算奖励和优势，并执行策略更新。更新后的模型参数随后被同步回推理引擎，为下一轮迭代做准备。
...
- **新的分布式训练策略**：可以通过扩展 strategy 基类来实现新的分布式训练方法。

- **自定义 RL 算法**：不同的强化学习算法可以实现为新的 trainer 类。

- **推理优化**：可以通过 strategy 模块集成额外的推理引擎，并持续集成 vLLM 和 SGLang 等推理引擎的最新版本特性。
