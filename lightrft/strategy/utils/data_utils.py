...
class DistributedSampler(Sampler[_T_co]):
    ...
    :type num_replicas: Optional[int]
    ...
    :param shuffle: If True (default), sampler will shuffle the indices.
    :type shuffle: bool
    ...
    :type seed: int
    :param drop_last: If True, then the sampler will drop the tail of the data to make it evenly
                    divisible across the number of replicas. If False, the sampler will add
                    extra indices to make the data evenly divisible across the replicas.
    :type drop_last: bool
    :param consumed_samples: Number of samples that have been consumed already, useful for resuming training.
    ...
    def __init__(  # pylint: disable=W0231, R0917
        self,
        dataset: Dataset,
        ...
        :param rank: Rank of the current process within num_replicas.
        :type rank: Optional[int]
        :param shuffle: If True, sampler will shuffle the indices.
        :type shuffle: bool
        :param seed: Random seed used for shuffling.
        :type seed: int
        :param drop_last: If True, drop the last incomplete batch.
        :type drop_last: bool
        :param consumed_samples: Number of samples already consumed, for resuming training.
        :type consumed_samples: int
        :raises RuntimeError: If distributed package is not available but required.
        :raises ValueError: If rank is invalid for the given number of replicas.
        """
    ...
    def __len__(self) -> int:
        ...
        :return: The number of samples in this sampler.
        :rtype: int
        """
    ...
    def set_epoch(self, epoch: int, consumed_samples=0) -> None:
        ...
        for each epoch. Otherwise, the next iteration of this sampler will yield
        the same ordering.

        :param epoch: Epoch number.
        :type epoch: int
        :param consumed_samples: Number of samples already consumed in this epoch.
        :type consumed_samples: int
        """
        self.epoch = epoch
        self.consumed_indicies = consumed_samples // self.num_replicas
