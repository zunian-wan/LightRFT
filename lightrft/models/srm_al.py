...
class ScalarRewardModelAL(nn.Module):
    """
    Scalar Reward Model for reinforcement learning applications with audio-language backbones.

    This class wraps around a pretrained audio-language model and adds reward heads to produce scalar scores.
    ...
    :param pooling_method: Pooling method for aggregating hidden states ('attn' or 'last').
                           Default to 'attn', which delivers best performance.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(pretrain_or_model, str):
            ...
            # Flash Attention 2 issue with Qwen2-Audio: https://github.com/QwenLM/Qwen2-Audio/issues/51
            if "qwen2" in pretrain_or_model.lower():
            ...
            # Set the probing layer for hidden states probing
            ...
            for head_type in head_types:  # e.g., "alignment", "preference"
                head = nn.Sequential(
    ...
    def forward(
        ...
        """
        The forward takes sequences and audio content as input and returns a dictionary
        ...
        :param input_features: Preprocessed audio features of input audio
        :type input_features: torch.Tensor
        :param feature_attention_mask: Attention mask for the audio features
        :type feature_attention_mask: Optional[torch.Tensor]
